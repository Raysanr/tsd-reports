                ...
                <div class="space-y-3">
                    ...
                    <div class="line-item-row flex items-center gap-3 flex-wrap" data-variation-id="{{ $variationId }}" data-name="{{ $name }}" data-price="{{ $price }}" data-qty="{{ $qty }}">
                        ...
                        <div class="min-w-[120px] flex-1 basis-40">
                            ...
                            @php
                                // Pancake POS's own PER-ITEM Assignee icon
                                // (explicit request, 2026-09-18: "i want
                                // the assignee is the per product like
                                // this because it is like in the pos ...
                                // but in the call tracker the 2 product
                                // including upsell it has no assignee in
                                // that, it should be like in the pos").
                                // Confirmed live against real production
                                // orders: each items[] entry carries its
                            ...
                            <div class="relative mt-1 inline-assignee-wrap" data-lead-id="{{ $lead->id }}" data-variation-id="{{ $variationId }}">
                                <button type="button" class="inline-assignee-btn flex items-center gap-1.5 text-xs text-slate-400 hover:text-primary-dark cursor-pointer">
                                    ...
                                    <span class="inline-assignee-label">
                                        @if(!empty($effectiveAssignee['name']))
                                        @if($effectiveAssignee['avatar_url'] ?? null)
                                        <img src="{{ $effectiveAssignee['avatar_url'] }}" alt="" class="inline w-4 h-4 rounded-full object-cover align-middle mr-1">
                                        @endif
                                        <span class="text-slate-600 dark:text-slate-300 font-medium">{{ $effectiveAssignee['name'] }}</span>
                                        @else
                                        No assigned staff
                                        @endif
                                    </span>
                                </button>
                                <div class="inline-assignee-panel hidden absolute z-20 mt-1 w-64 max-w-[90vw] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg">
                                    <input type="text" class="inline-assignee-search w-full text-xs border-b border-slate-100 dark:border-slate-700 px-3 py-2 bg-transparent text-slate-800 dark:text-slate-100 focus:outline-none" placeholder="Search staff…" autocomplete="off">
                                    <div class="inline-assignee-results max-h-48 overflow-y-auto">
