...
{{-- DAILY BREAKDOWN (explicit request, 2026-08-14: shaped like the source
     sheet's own daily tally) — one row per calendar day across the page's
     existing date-range picker, same disposition columns as the Hourly
     Breakdown above (reuses $displayCols) but bucketed by day instead of
     hour-of-day, plus a Gross Sales column (buildRow()'s 'upsell_sales' —
     same figure the AOV KPI card above is derived from, kept consistent
     rather than inventing a second "gross sales" definition). Every
     calendar day in range gets a row, including all-zero rest days —
     unlike the hourly table, a multi-day table reads as an activity log
     over time, where a zero day IS the signal, not noise to hide, so
     $dailyRows always has one entry per day (no empty-state branch needed
     the way the hourly table's does). --}}
<div class="mt-6 overflow-auto bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm" style="max-height:calc(100vh - 380px)" data-scroll-shadow>
    <table class="w-full border-collapse text-xs font-mono" style="min-width:1280px">
        <thead class="sticky top-0 z-20 shadow-sm">
            <tr>
                <th rowspan="2"
                ...
                <th rowspan="2"
                    class="bg-emerald-100 dark:bg-emerald-900 border border-slate-300 dark:border-slate-600 px-3 py-2.5 text-center text-[11px] font-bold text-emerald-900 dark:text-emerald-200 uppercase tracking-wide leading-tight"
                    style="min-width:110px">
                    Gross Sales
                </th>
            </tr>
        ...
        <tbody>
            ...
            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                ...
                @endforeach
                <td class="border border-slate-200 dark:border-slate-700 px-2 py-2.5 text-center font-semibold text-emerald-700 dark:text-emerald-400">
                    ₱{{ number_format($day['row']['upsell_sales'], 2) }}
                </td>
            </tr>
            @endforeach
            ...
            {{-- RANGE TOTAL row --}}
            <tr class="bg-slate-900 text-white font-bold">
                <td class="sticky-col sticky-col-footer border border-slate-700 px-3 py-3 uppercase tracking-wider text-[11px]">Range Total</td>
                @foreach($displayCols as $col)
                <td class="border border-slate-700 px-2 py-3 text-center {{ !empty($col['highlight']) ? 'text-green-300' : '' }}">
                    {{ $summary[$col['key']] ?: '' }}
                </td>
                ...
                <td class="border border-slate-700 px-3 py-3 text-center text-emerald-300">
                    ₱{{ number_format($summary['upsell_sales'], 2) }}
                </td>
            </tr>
        </tbody>
    </table>
