...
// ─── TSA Performance (ALL view): per-team Grand Total breakdown ──────────────
// Explicit request, 2026-09-10: clicking a Grand Total cell shows that same
// metric split by team, not an order-ID list — a different popover CONTENT
// than the drilldown one above, so it gets its own small self-contained
// state/closure rather than reusing that one's `popover` variable (mixing
// the two would mean whichever handler runs last always wins/clobbers the
// other's popover reference). Data comes from data-gt-by-team, one shared
// JSON blob on the <tfoot> row (TsaPerformanceController::indexAll()'s own
// $grandTotalByTeam) — no separate fetch needed, unlike the drilldown above,
// since this is already computed server-side and small (one row per team).
(function () {
    let gtPopover = null;
    ...
    document.addEventListener('click', (e) => {
        const cell = e.target.closest('[data-gt-key]');
        if (!cell) {
            if (gtPopover && !gtPopover.contains(e.target)) closeGtPopover();
            return;
        }

        const row = cell.closest('[data-gt-by-team]');
        if (!row) return;

        // Toggle: clicking the same cell again closes it instead of
        // re-showing the identical popover.
        const wasOpenForThisCell = gtPopover?.dataset.forKey === cell.dataset.gtKey;
        closeGtPopover();
        if (wasOpenForThisCell) return;

        let byTeam;
        try {
            byTeam = JSON.parse(row.dataset.gtByTeam);
        } catch {
            return;
        }

        const key    = cell.dataset.gtKey;
        const label  = cell.dataset.gtLabel || key;
        const suffix = cell.dataset.gtSuffix || '';

        gtPopover = document.createElement('div');
        gtPopover.dataset.forKey = key;
        gtPopover.className = 'fixed z-50 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 py-1 text-xs font-mono';
        gtPopover.style.minWidth = '200px';
        gtPopover.innerHTML = `
            <p class="px-3 py-2 text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800 uppercase tracking-wide text-[10px] font-bold">${escapeHtml(label)} by team</p>
            ${Object.values(byTeam).map((team) => {
                const value = team.row[key];
                const display = value === null || value === undefined ? '—' : `${value}${suffix}`;
                return `
                    <div class="flex items-center justify-between gap-4 px-3 py-1.5 border-b border-slate-100 dark:border-slate-800 last:border-b-0">
                        <span class="text-slate-600 dark:text-slate-300">${escapeHtml(team.label)}</span>
                        <span class="font-bold text-slate-800 dark:text-slate-100">${escapeHtml(display)}</span>
                    </div>
                `;
            }).join('')}
        `;
        document.body.appendChild(gtPopover);

        const rect = cell.getBoundingClientRect();
        const maxLeft = window.innerWidth - 220;
        gtPopover.style.top  = `${rect.bottom + 4}px`;
        gtPopover.style.left = `${Math.max(8, Math.min(rect.left, maxLeft))}px`;
    });

    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') closeGtPopover();
    });

    document.addEventListener('scroll', (e) => {
        if (gtPopover?.contains(e.target)) return;
        closeGtPopover();
    }, true);
})();
