 ...
 * checked to the tsa management") — the same-team-first / cross-team-
 * fallback split (2026-09-08) is gone; which team a TSA is on no longer
 * matters at all here, only whether she's online and checked for the
 * lead's own product in TSA Management. Down to 2 tiers:
 *   1. Online (any team), checked for this lead's product
 *   2. Online (any team), ANY product — "someone answers the phone" as
...
{
    /**
     * Splits $tsa's own uncalled backlog across every other currently-
     * working TSA regardless of team, preferring one who actually handles
     * each lead's own product (see the class-level doc comment's 2-tier
     * fallback chain). Round-robins independently within whichever tier a
     * given lead lands in, so an uneven split within that tier lands the
     * extra lead on the first-picked TSAs rather than all piling onto one
     * — but two leads that land in DIFFERENT tiers (e.g. one has an
     * eligible TSA online, another doesn't) are never forced into the
     * same rotation together. No-ops silently (same "nothing to do"
     * convention as everywhere else in this app) when there's no backlog,
     * or nobody online at all to hand it to — she keeps it, it's not left
     * orphaned.
     *
    ...
    public static function redistribute(TsaShift $tsa, ?PancakeOrderTagApi $api = null): int
    {
        ...
        $online = TsaShift::where('id', '!=', $tsa->id)
            ->where('active', true)
        ...
        foreach ($backlog as $lead) {
            ...
            $tier1 = $online->filter(fn ($t) => in_array($t->id, $eligibleTsaIds, true))->values();
            ...
            if ($pool->isEmpty()) {
                continue; // nobody online at all — she keeps this one
            }
            ...
            // move used — makes it obvious later why a TSA ended up with a
            // lead for a product she isn't checked for (tier 2).
            $tierNote = match ($tier) {
                1 => '',
                2 => ' (nobody checked for this product was online — any-online fallback)',
            };
