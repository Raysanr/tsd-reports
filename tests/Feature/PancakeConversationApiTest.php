...
class PancakeConversationApiTest extends TestCase
{
    ...
    public function test_send_message_posts_a_reply_inbox_action(): void
    {
        PancakePageToken::create(['page_id' => '123', 'page_access_token' => 'cached-token']);
        Http::fake([
            'pages.fm/api/public_api/v1/pages/123/conversations/conv-1/messages*' => Http::response(
                ['success' => true, 'id' => 'msg-1'], 200
            ),
        ]);

        $result = $this->api->sendMessage('123', 'conv-1', 'Hi, confirming your order!');

        $this->assertTrue($result['success']);
        $this->assertNull($result['error']);
        Http::assertSent(function ($request) {
            return $request->method() === 'POST'
                && str_contains($request->url(), '/pages/123/conversations/conv-1/messages')
                && $request['action'] === 'reply_inbox'
                && $request['message'] === 'Hi, confirming your order!'
                && $request['page_access_token'] === 'cached-token';
        });
    }

    public function test_send_message_fails_gracefully_without_a_configured_token(): void
    {
        Setting::set('pancake_access_token', '');

        $result = $this->api->sendMessage('123', 'conv-1', 'Hello');

        $this->assertFalse($result['success']);
        $this->assertNotNull($result['error']);
    }

    public function test_send_message_surfaces_pancakes_rejection_eg_outside_the_24_hour_window(): void
    {
        PancakePageToken::create(['page_id' => '123', 'page_access_token' => 'cached-token']);
        Http::fake([
            'pages.fm/api/public_api/v1/pages/123/conversations/conv-1/messages*' => Http::response(
        ...
        $this->assertFalse($result['success']);
        $this->assertSame('Outside messaging window', $result['error']);
    }

    public function test_send_message_retries_with_a_fresh_token_on_a_stale_token_response(): void
    {
        PancakePageToken::create(['page_id' => '123', 'page_access_token' => 'stale-token']);
        Http::fake([
            'pages.fm/api/public_api/v1/pages/123/conversations/conv-1/messages*' => Http::sequence()
                ->push(['success' => false, 'error_code' => 105, 'message' => 'access_token renewed please use new access_token'], 200)
                ->push(['success' => true, 'id' => 'msg-1'], 200),
            'pages.fm/api/v1/pages/*/generate_page_access_token*' => Http::response(['page_access_token' => 'fresh-token'], 200),
        ]);
