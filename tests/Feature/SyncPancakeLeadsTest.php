...
class SyncPancakeLeadsTest extends TestCase
{
    ...
    /**
     * Real production incident, 2026-09-22: a deploy's container restart
     * killed an in-flight detached background sync (see handle()'s own
     * doc comment on why pancake:sync-leads runs detached via exec('...
     * &') from CronController — a container SIGTERM/SIGKILL never lets
     * that process reach the finally block that clears
    ...
    public function test_a_running_flag_under_two_minutes_old_still_blocks_a_second_run(): void
    {
        ...
        // Skipped, not run — last_run must be untouched by this call
        // (only a real run through doSync() would have overwritten it to
        // "just now").
        $this->assertSame($orphanedLastRun, Setting::get('pancake_sync_leads_last_run'));
        // A skip records no LeadSyncRun row at all (mirrors the existing
        // "API key not configured" early-return, the only OTHER
        // recordRun()-free path — this is the second).
        $this->assertNull(LeadSyncRun::latest('id')->first());
    }
}
